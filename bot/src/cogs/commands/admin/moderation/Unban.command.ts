...
export default class UnbanUser extends CommandCog {
  constructor() {
    super({
      name: "unban",
  ...
  async run(args: CommandRunArgs) {
    ...
    try {
      await guild.bans.remove(userIdAsString, reason?.toString());
    } catch {
    ...
    if (guildSettings?.logChannelId) {
      ...
      if (channel && channel.isText() && !channel.isThread()) {
        await sendLogMessage({
          ...
          title: "User unbanned",
          message: `User <@${guildMember?.user.tag || userId}> unbanned`,
        });
      }
    }
